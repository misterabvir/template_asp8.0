...
{
    public record Notification(Guid UserId, string Email, string Username, string Reason, string Code) : INotification;
    public class Handler(IUserRepository userRepository, IMessageRepository messageRepository, IEmailSender emailSender) : INotificationHandler<Notification>
    {
        public async Task Handle(Notification notification, CancellationToken cancellationToken)
        {
            ...
            await emailSender.SendVerificationEmailAsync(user.Email, user.Username, notification.Code);
            await messageRepository.AddMessageAsync(user.UserId, Constants.Types.Verification, notification.Reason , cancellationToken);
        }
    }
}
